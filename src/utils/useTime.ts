 ...
 * @returns 格式化後的時間字符串
 */
export function convertTime(date: any, isUTC = false) {
  const d = new Date(date)
  if (isNaN(d.getTime())) return null
 ...
 * 把傳入的毫秒轉成年月日時
 */
export function convertStartEnd(date: any, type: "start" | "end") {
  const d = new Date(date)
  if (isNaN(d.getTime())) return null
 ...
 * @returns [最小日, 最大日]
 */
export function getThisDateRange(offset: number | 0): [Date, Date] {
  const now = new Date()
 ...
 * @returns [最小日, 最大日]
 */
export function getThisYearRange(offset: number = 0): [Date, Date] {
  const d = new Date()
